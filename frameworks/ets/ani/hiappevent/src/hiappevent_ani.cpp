...
ani_double HiAppEventAni::AddProcessor(ani_env *env, [[maybe_unused]] ani_object object, ani_object processor)
{
    int64_t id = 0;
...
ani_object HiAppEventAni::Write(ani_env *env, [[maybe_unused]] ani_object object, ani_object info)
{
    std::string domain = "";
    HiAppEventAniHelper hiAppEventAniHelper;
    if (!hiAppEventAniHelper.GetPropertyDomain(info, env, domain)) {
        HILOG_ERROR(LOG_CORE, "GetPropertyDomain Failed");
        return hiAppEventAniHelper.WriteResult(env, hiAppEventAniHelper.BuildErrorByResult(ERR_PARAM));
    }

    std::string name = "";
    if (!hiAppEventAniHelper.GetPropertyName(info, env, name)) {
        HILOG_ERROR(LOG_CORE, "GetPropertyName Failed");
        return hiAppEventAniHelper.WriteResult(env, hiAppEventAniHelper.BuildErrorByResult(ERR_PARAM));
    }

    int32_t enumValue = 0;
    if (!hiAppEventAniHelper.GeteventTypeValue(info, env, enumValue)) {
        HILOG_ERROR(LOG_CORE, "GeteventTypeValue Failed");
        return hiAppEventAniHelper.WriteResult(env, hiAppEventAniHelper.BuildErrorByResult(ERR_PARAM));
    }
    ...
    if (!hiAppEventAniHelper.ParseParamsInAppEventPack(env, paramTemp, appEventPack)) {
        HILOG_ERROR(LOG_CORE, "ParseParamsInAppEventPack Failed");
...
ANI_EXPORT ani_status ANI_Constructor(ani_vm *vm, uint32_t *result)
{
    ...
    ani_class cls {};
    if (ANI_OK != env->FindClass(CLASS_NAME_APPEVENT, &cls)) {
